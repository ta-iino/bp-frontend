<template>
  <div id="app">
        ...
        <v-col cols="4" class="px-0 d-flex">
          <div class="ml-n8 mr-2">
            <v-btn class="v-btn" depressed color="light-blue-darken-4" border="0"
              @click="matchingStart()" :disabled="activeBtn">ニーズマッチング</v-btn>
          </div>
          <div class="px-2">
    ...
    <!-- ヘッダ部分 ここまで -->
    <!-- 一覧表示 ここから -->
    <v-container class="ui-vcontaoner pt-0 mb-4" v-if="destinationCompanies && destinationCompanies.length" >
      <v-data-table
        v-model:page="page"
      ...
      <template v-slot:[`item.industry1`]="{ item }">
        {{ getIndustry(item.raw.id, 0) }}
      </template>
      <template v-slot:[`item.industry2`]="{ item }">
        {{ getIndustry(item.raw.id, 1) }}
      </template>
      <template v-slot:[`item.industry3`]="{ item }">
        {{ getIndustry(item.raw.id, 2) }}
      </template>
      <template v-slot:[`item.businessItems`]="{ item }">
        {{ getTsrData(item.raw.id, '営業種目') }}
      </template>
      <template v-slot:[`item.representativeAge`]="{ item }">
        {{ getCeoAge(item.raw.tsr['生年月日']) }}
      </template>
      <template v-slot:[`item.numberOfEmployee`]="{ item }">
...
const sendCompanyIds: any = new Array();
...
const getTsrData = (id: any, targetKey: any) => {
  ...
  let result = (destinationCompanies.value).filter((destinationCompanyData: any) => id === destinationCompanyData.id)[0]
  if(result) {
...
const getIndustry = (id: any, index: any) => {
  let result = (destinationCompanies.value).filter((destinationCompanyData: any) => id === destinationCompanyData.id)[0]
  let key = Object.keys(result.industry)[index]
  if(key) {
...
const putHyphen = (zip: any) => {
  let startNum = zip.substring(0, 3)
  let endNum = zip.substring(3,7)
  let newZip = startNum + '-' + endNum
  return newZip;
}
